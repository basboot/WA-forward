...
class WAForwarder {
  ...
  private async init() {
    ...
    const job = schedule.scheduleJob('0 0 12 * * *', function(){
      ...
      this.sendForwarderState(this.client, "Still alive");
    });

    // inform remote phone the script is active
    this.sendForwarderState(this.client, "Started");
    ...
    const prods = await this.client.getBusinessProfilesProducts(me.wid)
    ...
    this.client.onMessage(async message => {
            ...
            if (message.quotedMsg != null) {
              ...
              switch (command) {
                ...
                case "forward":
                  ...
                  this.sendForwarderState(this.client, "Ready");
                  return;
                  break;
                case "relay":
      ...
      if ((!messageFromMe) || (this.ForwarderState.test)) {
        try {  
          ...
          if (message.quotedMsg != null) {
            ...
            if (message.quotedMsg.type == MessageTypes.TEXT) {
              if (message.isGroupMsg) {
                txtMessage = `${txtMessage} _(${this.formatSenderName(message.quotedMsg.sender)}: "${message.quotedMsg.body.replace(/\*|\_/gi,'')}")_`
              } else {
          ...
          // Set message sender
          senderMessage = this.formatSenderName(message.sender);
  ...
  private sendForwarderState(client:Client, message: string) {
  ...
  */
  private formatSenderName(sender) {
    // Set message sender
